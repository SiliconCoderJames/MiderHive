...
> **同一维度只能有一个模型（维度↔provider 绑定）**：同一维度的向量在 vec0 里
> **共用一张表**，因此不同模型的向量会互相污染 kNN 结果（查询时无法按 provider 过滤）。
> 平台在该维度**首次写入时登记其 provider**（内置 384 维在启动时登记为 `ngram-hash`），
> 之后同维度换模型的写入直接返回 **400**，错误里点明已绑定的 provider——
> 把“检索结果莫名其妙”变成一眼可读的用法错误。要用第二个模型，请让它输出
> **另一个维度**。存量库中已有的混杂数据不受影响（新校验只管后续写入）。
...
不敏感），更短的查询回退 LIKE。**全文索引只覆盖每个条目的最新版本**——追加新版本时
旧版本自动退出索引，因此索引体积不随历史版本数增长。
语义模式返回每项 `score`（距离，越小越相关）。
带 `tag` 的语义检索采用"多召回再过滤"（按梯度扩 k 直至凑够 `limit` 或该维度取尽）。
