#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""把请求延迟分层：服务端每请求成本 vs 连接建立 vs 检索本身。

背景：本仓库的性能报告初版曾把 bench_search 的 P95≈50ms 解读成"约 40ms 的请求地板"。
这个解读**被本脚本证伪**（服务端 p50 只有 0.2–0.3 ms），因此本脚本的用途是
分层量测而非验证某个既定结论。区分三种假设：
